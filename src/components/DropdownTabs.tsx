...
export default function DropdownTabs<T extends string>({
  ...
  const FaceContent = ({
    ...
    <div
      className={`rounded-[22px] mobile:rounded-[18px] p-1 ${($transparentBg && !open) || open ? 'bg-transparent' : 'bg-cyberpunk-card-bg'
        }`}
    >
      <Row
        ...
        style={
          isValueSelected
            ? {
              background: 'linear-gradient(245.22deg, rgb(240 74 68), rgb(43, 106, 255), rgb(57, 208, 216))',
      ...
      >
        <Row className="min-w-[120px] items-stretch justify-between mobile:min-w-[108px] h-9 mobile:h-7">
          <Row
            onClick={onClickFace}
            className={`grow-2 justify-center items-center ${isValueSelected ? 'text-white' : 'text-[#abc4ff]'
  ...
  return (
    <div className={twMerge('relative', className)}>
      ...
      <Collapse
        ...
        className={(open) =>
          `absolute z-dropdown top-0 left-0 w-full ${open ? 'bg-cyberpunk-card-bg' : 'bg-transparent'
        ...
        <Collapse.Body>
          <RadioGroup
            ...
            itemClassName={(checked) =>
              twMerge(
                `my-3 px-3 text-sm mobile:text-xs font-medium whitespace-nowrap ${checked ? 'text-white' : 'text-[#ABC4FF]'
                }`,
