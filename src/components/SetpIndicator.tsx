        ...
        <Row key={index}>
          ...
          <Col className="items-center">
            {shrinkToValue(renderStepNumber, [info]) || (
              <div
                className={`grid place-items-center h-8 w-8 mobile:h-6 mobile:w-6 text-sm font-medium bg-[#04133d] rounded-full ${currentStep === info.stepNumber ? 'text-[#F1F1F2]' : 'text-[rgba(171,196,255,.5)]'
          ...
          <div className="ml-2">
            {shrinkToValue(renderStepContent, [info]) || (
              <div
                className={`text-sm font-medium ${currentStep === info.stepNumber ? 'text-[#F1F1F2]' : 'text-[rgba(171,196,255,.5)]'
                  } pt-1.5`}
              >
                {info.stepContent}
              </div>
