    ...
    <Row className={twMerge('gap-4', className)}>
      {configs?.map((config) => {
        ...
        return (
          <div
            key={config.id}
            className={`relative grow items-stretch p-3 gap-2 ${isCurrent ? 'ring-inset ring-1.5 ring-[#abc4ff]' : 'ring-inset ring-1.5 ring-[#abc4ff60]'
          ...
          >
            {isCurrent ? (
              <div className="absolute p-0.5 rounded-full right-1 top-1 -translate-y-1/2 translate-x-1/2 z-10 bg-[#04133d] text-[#abc4ff]">
