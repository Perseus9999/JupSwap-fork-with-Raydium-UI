...
export default function AddNewReward(props: Props) {
  ...
  return (
    <Card
      className={`p-4 mb-4 mobile:p-3 bg-cyberpunk-card-bg border-1.5 border-[rgba(171,196,255,0.2)] ${isMobile ? 'rounded-2xl' : 'rounded-3xl'
        }`}
      size="lg"
    >
      <Grid className="gap-4">
        ...
        {topError ? <div className="text-[#f04a44] text-right text-sm font-medium px-2">{topError}</div> : null}
          ...
          {hasInput && (
            <div>
              {gt(newReward.duration, MAX_DURATION) ? (
                <div className="text-[#f04a44] text-sm font-medium  pl-2">
