      ...
      <ListTable
        ...
        renderRowItem={({ item: reward, label, index }) => {
          ...
          const getRewardBadge = () => {
            if (isRewardEnded)
              return (
                <Badge className="w-fit" cssColor="#f04a44">
                  Ended
                </Badge>
              )
        ...
        renderItemActionButtons={({ itemData: reward, index }) => {
          ...
          if (!isRewardEnded && !previewMode) {
            return (
              <div
                className={`flex bg-[#abc4ff1a] mobile:bg-transparent items-center rounded-md p-2 mobile:p-0 mb-4 mobile:mb-0 empty:hidden ${canAddMore ? '' : 'opacity-50 cursor-not-allowed'
                  }`}
              >
                <Button
                  disabled={!canAddMore}
            ...
            return (
              <div className="flex bg-[#abc4ff1a] mobile:bg-transparent items-center rounded-md p-2 mobile:p-0 mb-4 mobile:mb-0 empty:hidden">
                ...
                <Button
                  ...
                  className={`flex flex-1 gap-1 justify-center items-center text-secondary-title text-xs font-medium ${hasUnClaimed && !isApprovePanelShown ? 'clickable' : 'cursor-default opacity-50'
                    } mobile:py-4`}
                >
