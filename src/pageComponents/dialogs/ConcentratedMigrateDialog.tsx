...
export default function ConcentratedMigrateDialog({
  ...
  return (
    ...
    >
      {({ close: closeDialog }) => (
        <Card
          className={`p-6 mobile:p-4 rounded-3xl mobile:rounded-lg ${canShowMigrateDetail ? 'w-[min(600px,90vw)]' : 'w-[min(450px,90vw)]'
        ...
        {mode === 'custom' && (
          <div>
            <Grid className="grid-cols-2-fr gap-3">
              <Row
                className={`border-1.5 ${isPriceRangeInRange && isPriceRangeValid ? 'border-[#abc4ff40]' : 'border-[#f04a44]'
            ...
            {!isPriceRangeValid ? (
              <div className="text-[#f04a44] text-sm mt-1">This range is invalid.</div>
            ) : !isPriceRangeInRange ? (
              <div className="text-[#f04a44] text-sm mt-1">The current price is out of this range.</div>
            ) : null}
          </div>
        )}
      </div>
...
function ModeItem({
  ...
  return (
    <div
      onClick={onClick}
      className={`relative border-1.5 ${selected ? 'border-[#39d0d8]' : 'border-[#abc4ff40]'
        } rounded-xl py-3 px-4 bg-[#04133d] cursor-pointer`}
    >
