  ...
  const avaliableTokenAmount = useMemo(
    () =>
      stakeDialogMode === 'deposit'
        ? stakeDialogInfo?.poolData?.quote.address && balances[String(stakeDialogInfo?.poolData?.quote.address)]
            ...
            <Button
              ...
              onClick={() => {
                ...
                const tokenAmount = toTokenAmount(inputToken, amount, { alreadyDecimaled: true })
                  ; (stakeDialogMode === 'withdraw'
                    ? txUnStake(new web3.PublicKey(stakePoolAddress), new BN(parseFloat(amount) * 10 ** inputTokenDecimal), new web3.PublicKey(inputTokenAddress), inputTokenDecimal, inputToken.name)
                    : program && txStake(program, new web3.PublicKey(stakePoolAddress), new BN(parseFloat(amount) * 10 ** inputTokenDecimal), new web3.PublicKey(inputTokenAddress), inputTokenDecimal, inputToken.name)
                  )
              }}
            >
              {stakeDialogMode === 'withdraw' ? `Unstake ${stakeDialogInfo?.poolData?.name}` : `Stake ${stakeDialogInfo?.poolData?.name}`}
