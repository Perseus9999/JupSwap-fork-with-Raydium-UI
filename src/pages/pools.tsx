...
export default function PoolsPage() {
  ...
  return (
    <PageLayout contentButtonPaddingShorter mobileBarTitle="Pools" metaTitle="Pools - BrrrChain">
      <PoolHeader />
      <PoolCard />
    </PageLayout>
  )
    ...
    <Input
      ...
      suffix={
        <Icon
          heroIconName="x"
          size={isMobile ? 'xs' : 'sm'}
          className={`text-[rgba(196,214,255,0.5)] transition clickable ${storeSearchText ? 'opacity-100' : 'opacity-0 pointer-events-none'
            }`}
          onClick={() => {
        ...
        <Popover.Panel>
          <div>
            ...
            >
              <Grid className="grid-cols-1 items-center gap-2 ">
                <DecimalInput
                  className={`px-3 py-2 mobile:py-1 ring-inset ring-1 ${filterValueUnavailable ? 'ring-[rgba(255,86,86,0.94)]' : 'ring-[rgba(196,214,255,0.5)]'
...
function PoolCardDatabaseBodyCollapseItemFace({
  ...
  const pcCotent = (
    <Row
      type="grid-x"
      className={`py-5 mobile:py-4 mobile:px-5 bg-[#04133d] items-center gap-2 grid-cols-[auto,1.6fr,1fr,1fr,1fr,.8fr,auto] mobile:grid-cols-[1fr,1fr,1fr,auto] rounded-t-3xl mobile:rounded-t-lg ${open ? '' : 'rounded-b-3xl mobile:rounded-b-lg'
        } transition-all`}
    >
  ...
  const mobileContent = (
    <Collapse open={open}>
      <Collapse.Face>
        <Row
          type="grid-x"
          className={`py-3 px-3 items-center gap-2 grid-cols-[auto,1.5fr,1fr,1fr,auto] bg-[#04133d] mobile:rounded-t-lg ${open ? '' : 'rounded-b-3xl mobile:rounded-b-lg'
            }`}
        >
      ...
      <Collapse.Body>
        <Row
          type="grid-x"
          className="py-4 px-5 pl-12 relative items-center gap-2 grid-cols-[1.5fr,1fr,1fr,auto]  bg-[#04133d]"
        >
          ...
          <>
            ...
            <Tooltip>
              <Icon
                ...
                className={`grid place-items-center w-10 h-10 mobile:w-8 mobile:h-8 ring-inset ring-1 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-xl mobile:rounded-lg text-[rgba(171,196,255,.5)] clickable-filter-effect ${correspondingFarm ? 'clickable' : 'not-clickable'
                  }`}
                onClick={() => {
                  routeTo('/farms', {
            ...
            <Tooltip>
              <Icon
                size="smi"
                iconSrc="/icons/pools-remove-liquidity-entry.svg"
                className={`grid place-items-center w-10 h-10 mobile:w-8 mobile:h-8 ring-inset ring-1 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-xl mobile:rounded-lg text-[rgba(171,196,255,.5)] ${hasLp ? 'opacity-100 clickable clickable-filter-effect' : 'opacity-50 not-clickable'
                  }`}
                onClick={() => {
                  hasLp &&
                    routeTo('/liquidity/add', {
