...
export default function StakingPage() {
  return (
    <PageLayout mobileBarTitle="Staking" metaTitle="Staking - BrrrChain" contentButtonPaddingShorter>
      <StakingHeader />
      <StakingCard />
...
function StakingCard() {
  ...
  const infos = useStaking((s) => s.info)
    ...
    <CyberpunkStyleCard
      ...
      {/* {!isMobile && (
        <Row
          ...
          <div className=" font-medium self-center text-[#ABC4FF] text-sm">Staked Amount</div>
...
function StakingCardCollapseItemFace({ open, info }: { open: boolean; info: IStakeInfo }) {
  ...
  const pcCotent = (
    <Row
      type="grid-x"
      className={`bg-[#04133d] py-5 px-8 mobile:py-4 mobile:px-5 gap-2 items-stretch grid-cols-[1fr,1fr,1fr,1fr,1fr,auto] mobile:grid-cols-[1fr,1fr,1fr,1fr,auto] rounded-t-3xl mobile:rounded-t-lg ${open ? '' : 'rounded-b-3xl mobile:rounded-b-lg'
        }`}
    >
      <Row className="font-medium text-[#ABC4FF] text-sm items-center no-clicable-transform-effect">
        {info.poolData && <img src={`/tokens/${info.poolData?.quote.address}.png`} style={{ width: "68px", height: "32px" }} />}
        {info.poolData?.name}
      </Row>
      ...
      <TextInfoItem
        ...
        subValue={`${info.stakeEntryData ? "$" + new Intl.NumberFormat().format(info.stakeEntryData.pendingReward * (info.rewardTokenPrice ?? 0)) : "--"}`}
      />

      <TextInfoItem
        name="Staked"
        value={info.stakeEntryData && new Intl.NumberFormat().format(info.stakeEntryData.stakeBalance)}
        subValue={`${info.stakeEntryData ? "$" + new Intl.NumberFormat().format(info.stakeEntryData.stakeBalance * (info.quoteTokenPrice ?? 0)) : "--"}`}
      />

      <TextInfoItem
        name="APR"
        value={`${new Intl.NumberFormat().format(info.apr ?? 0)} %`}
      />

      <TextInfoItem
        name="Total Staked"
        value={info.stakePoolData && new Intl.NumberFormat().format(info.stakePoolData.balance)}
        subValue={`${info.stakePoolData ? "$" + new Intl.NumberFormat().format(info.stakePoolData.balance * (info.quoteTokenPrice ?? 0)) : "--"}`}
      />
  ...
  const mobileContent = (
    <Collapse open={open}>
      <Collapse.Face>
        <Row
          type="grid-x"
          className={`py-4 px-5 items-center gap-2 grid-cols-[1fr,1fr,auto] mobile:rounded-t-lg ${open ? '' : 'rounded-b-3xl mobile:rounded-b-lg'
            }`}
        >
          <Row className="font-medium text-[#ABC4FF] text-sm items-center no-clicable-transform-effect">
      ...
      <Row className="p-6 mobile:py-3 mobile:px-4 flex-grow ring-inset ring-1.5 mobile:ring-1 ring-[rgba(171,196,255,.5)] rounded-3xl mobile:rounded-xl items-center gap-3">
        <div className="flex-grow">
          ...
          <div className="text-white font-medium text-base mobile:text-xs">
            <a href={info.poolData?.getUrl} target='_blank' rel="noreferrer">{info.poolData?.name}</a>
          </div>
          <div className="text-[rgba(171,196,255,0.5)] font-medium text-sm mobile:text-xs">
            {info.stakeEntryData?.stakeBalance}
          </div>
        </div>
        <Row className="gap-3">
      ...
      >
        <Row className="flex-grow divide-x-1.5 w-full">
          ...
          >
            ...
            <div className="text-white font-medium text-base mobile:text-xs">
              {new Intl.NumberFormat().format(info.stakeEntryData?.pendingReward ?? 0)} {`BRRR`}
            </div>
            <div className="text-[rgba(171,196,255,0.5)] font-medium text-sm mobile:text-2xs">
              ${new Intl.NumberFormat().format((info.stakeEntryData?.pendingReward ?? 0) * (info.rewardTokenPrice ?? 0))}
            </div>
          </div>
        </Row>
        <Button
