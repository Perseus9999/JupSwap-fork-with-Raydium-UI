    ...
    useNotification.getState().popConfirm({
      ...
      description: (
        <div className="space-y-4  text-center">
          ...
          <Row className="justify-center items-center gap-2 my-4 bg-[#04133d] rounded py-3 w-full">
  ...
  return (
    <FadeIn>
      {bothHaveAmount && !hasAcceptedPriceChange && !isApprovePanelShown && (
        <Row className="mt-5 bg-[#04133d] rounded-xl py-2 px-6 mobile:px-4 items-center justify-between">
  ...
  return (
    <FadeIn>
      {solBalance && lt(solBalance, SOL_BASE_BALANCE) && gte(solBalance, 0) && (
        <Row className="text-sm mt-4 text-[#e3dc8b] items-center justify-center">
  ...
  return (
    <Col>
      ...
      <FadeIn>
        {priceImpact ? (
          <div
            className={`font-medium text-xs whitespace-nowrap ${isDangerousPrice ? 'text-[#f04a44]' : isWarningPrice ? 'text-[#e3dc8b]' : 'text-[#39D0D8]'
              } transition-colors`}
          >
  ...
  return (
    ...
    >
      ...
      <SwapCardItem
        ...
        fieldValueTextColor={isDangerousPrice ? '#f04a44' : isWarningPrice ? '#e3dc8b' : '#39D0D8'}
        ...
        <Collapse.Body>
          <Col className="gap-3 pb-3">
            ...
            <SwapCardItem
              ...
              fieldValue={
                <Row className="py-1 px-2 bg-[#04133d] rounded-sm text-[#F1F1F2] font-medium text-xs -my-1">
                  <Input
                    className="w-7 px-0"
                    disabled={isApprovePanelShown}
            ...
            {slippageToleranceState === 'too small' && (
              <div className={`text-[#e3dc8b] text-xs mobile:text-sm`}>Your transaction may fail</div>
            )}
            {slippageToleranceState === 'too large' && (
              <div className={`text-[#e3dc8b] text-xs mobile:text-sm`}>
  ...
  return (
    <Row className={twMerge('grid gap-2 items-center grid-cols-[5em,1fr,auto,auto]', className)}>
      ...
      <AddressItem
        ...
        canCopy
        canExternalLink
        textClassName="flex text-xs font-normal text-white bg-[#04133d] rounded justify-center"
      >
        {address}
      </AddressItem>
    </Row>
      ...
      {canShowKline && (
        <div className="flex mobile:grid mobile:grid-cols-3 mobile:gap-1 p-4 mobile:p-2 w-[min(456px,100%)] self-center items-center">
          ...
          <Col className="items-start mobile:items-center mobile:justify-self-center ml-8  mobile:ml-0 w-8">
            ...
            <div
              className={`text-sm font-medium ${isPositive ? 'text-[#39D0D8]' : isNegative ? 'text-[#f04a44]' : 'text-[#abc4ff]'
                }`}
            >
...
function KLineChartItemThumbnail({
  ...
  const lineColor = isPositive ? '#39D0D8' : isNegative ? '#f04a44' : '#abc4ff'
